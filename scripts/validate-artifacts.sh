...
# HERMES_FOREGROUND_FAILURE_GUIDANCE.json, REPO_STAR_GENERICITY_PROOF_RECEIPT.json,
...
if [ -z "$SCHEMA_NAME" ]; then
  ...
  case "$BASENAME" in
    ...
    *)
      ...
      echo "Available: SCORECARD, OPPORTUNITIES, OPTIMIZATION_SCORECARD, COMMAND_OUTPUT_ROI_RECEIPT, FINDINGS, TOKEN_MEASUREMENT_SUMMARY, HOTSPOT_EVIDENCE_PACKETS, AGENTIC_ROOT_CAUSE_BRIEFS, TRANSFER_ORACLE_RECEIPT, CRITIQUE_RESULT, PLAYBOOK_MANIFEST_AUTHORITY, ADAPTER_AUDIT_SUMMARY, SOURCE_INSIGHT_PACKET, HERMES_FOREGROUND_RUN_RECEIPT, HERMES_FOREGROUND_FAILURE_GUIDANCE, REPO_STAR_GENERICITY_PROOF_RECEIPT, REPO_STAR_DOWNSTREAM_GENERICITY_PROOF_RECEIPT"
      exit 1
      ;;
  esac
fi
