...
check "contract requires fallback without memory" grep -Fq "Fallback without memory" "$CONTRACT"
...
check "contract keeps capture natural-only" grep -Fq "natural-only foreground capture" "$CONTRACT"
check "contract requires exact GBrain readback" grep -Fq "exact-read the slug back with" "$CONTRACT"
...
check "template includes fallback without memory" grep -Fq "Fallback without memory" "$TEMPLATE"
check "template includes natural-only capture posture" grep -Fq "Natural-only foreground capture" "$TEMPLATE"
