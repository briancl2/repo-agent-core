...
check "names current upstream docs/root instructions" grep -Fq "Current upstream docs/root instructions checked" "$CONTRACT"
...
check "requires native attempt or blocker" grep -Fq "safe documented native attempt or" "$CONTRACT"
check "requires verdict-bearing evidence" grep -Fq "verdict-bearing surface must carry" "$CONTRACT"
check "treats negative claims as verdicts" grep -Fq "not-production-ready, not-GA, or fallback-required" "$CONTRACT"
...
check "template includes upstream docs/root instructions" grep -Fq "Current upstream docs/root instructions checked" "$TEMPLATE"
check "template includes native evidence before verdict" grep -Fq "Native evidence before verdict" "$TEMPLATE"
check "template includes native attempt or blocker" grep -Fq "Native attempt or blocker" "$TEMPLATE"
