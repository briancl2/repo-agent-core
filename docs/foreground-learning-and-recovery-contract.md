...
2. Capture to GBrain only when the learning is reusable and decision-changing.
   Routine "no learning" notes, null searches, and validation success should use
   `No-capture reason`.
...
4. Self-healing is not a retrospective request by default. If the owner blocker
   is known, route to owner action first.
5. The contract does not authorize `gbrain sync --watch`, cron, jobs, minions,
...
that owns the truth, and names the owner action or explicitly says why no owner
action exists.
